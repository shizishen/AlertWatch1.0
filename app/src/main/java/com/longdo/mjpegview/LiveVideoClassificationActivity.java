...
public class LiveVideoClassificationActivity extends AbstractCameraXActivity<LiveVideoClassificationActivity.AnalysisResult> {
    ...
    static class AnalysisResult {
        ...
        protected void applyToUiAnalyzeImageResult(AnalysisResult result) {
            //方法体中的代码用于将`AnalysisResult`对象中的结果文本设置到`mResultView`（一个`TextView`）中，
            // 并调用`invalidate()`方法使得`mResultView`无效并触发重绘。
            mResultView.setText(result.mResults);
            mResultView.invalidate();
        }

        //`Bitmap`是Android中常用的图像数据类型，它可以包含图像的像素数据。
        // 通过将`Image`对象转换为`Bitmap`对象，可以方便地提取图像的像素数据，并将其传递给PyTorch进行处理。
